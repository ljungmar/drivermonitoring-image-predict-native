...
def load_and_preprocess_image(image, output_path, rows, cols, min_size=75):
    if isinstance(image, str):
        ...
        raise ValueError(
            "Unsupported image type. Only file paths or NumPy arrays are supported."
        )
    ...
    for epoch in range(epochs):
        for index in range(epoch_range):
            start = index * batch_size
                ...
                (index + 1) * batch_size, sample_count
            )  # Adjust end to handle the last batch
            x = images[start:end]
            y = labels[start:end]
            ...
            if saved_directory:
                ...
                for image_index in range(start, end):
                    image_path = os.path.join(
                        saved_directory, f"image_{image_index}_preprocessed.jpg"
                    )
            ...
            if data_augmentation is not None:
                ...
                for i in range(len(generator)):
                    ...
                    if len(y_augmented.shape) == 1:
                        ...
                        y_augmented_one_hot = to_categorical(
                            y_augmented, num_classes=num_classes
                        )
                    else:
                    ...
                    assert x_augmented.shape[1:] == (
                        rows,
                        cols,
                        3,
                    ), f"Expected 1 image with shape {(rows, cols, 3)}, but got {x_augmented.shape}"
                ...
                assert x.shape == (
                    ...
                    cols,
                    3,
                ), f"Expected {end - start} images with shape {(rows, cols, 3)}, but got {x.shape}"
                assert y_one_hot.shape == (
                    end - start,
                    num_classes,
                ), f"Expected {end - start} labels with shape ({num_classes},), but got {y_one_hot.shape}"

                yield x, y_one_hot


def training_with_dataaugmentation(
    images_train,
    labels_train,
    images_test,
    labels_test,
    ...
    pivot_train = int(
        len(images_train) * 0.8
    )  # Assuming 80% training and 20% validation split
    ...
    ds_vali = tf.data.Dataset.from_generator(
        lambda: batch_image_generator(
            images_val, y_val, batch_size, rows, cols, data_augmentation=train_datagen
        ),
        output_types=(tf.float32, tf.float32),
        output_shapes=([batch_size, rows, cols, 3], [batch_size, 2]),
    )
    ...
    ds_test = tf.data.Dataset.from_generator(
        lambda: batch_image_generator(
            images_test, y_test, batch_size, rows, cols, data_augmentation=test_datagen
        ),
        output_types=(tf.float32, tf.float32),
        output_shapes=([batch_size, rows, cols, 3], [batch_size, 2]),
    )
    ...
    try:
        test_loss, test_accuracy = model.evaluate(
            ds_test, steps=nb_test_samples // batch_size
        )
...
def main(image_path, datapath, modelname, rows, cols, batch_size, nb_epoch):
    ...
    trained_model, history = training_with_dataaugmentation(
        images_train,
        labels_train,
        images_test,
        labels_test,
    ...
    if class_labels.shape[1] != num_classes:
        print(
            f"Error: The model's output classes ({class_labels.shape[1]}) don't match the filtered class names ({num_classes})."
        )
        exit(1)
    ...
    result = {"image_path": image_path, "prediction": class_name}
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-p", "--image_path", required=True, help="Path to the uploaded image"
    )
    parser.add_argument(
        "-d",
        "--datapath",
        help="path of the dataset directory containing h5 files",
        required=True,
    )
    parser.add_argument(
        "-r", "--rows", help="row size, default 256", type=int, default=256
    )
    parser.add_argument(
        "-c", "--cols", help="column size, default 256", type=int, default=256
    )
    parser.add_argument(
        "-b", "--batch_size", help="batch size, default 64", type=int, default=64
    )
    parser.add_argument(
        "-e", "--nb_epoch", help="number of epoch, default 10", type=int, default=10
    )
    parser.add_argument(
        "-m",
        "--model",
        help="model name, choose cnn",
        choices=["cnn"],
        type=str,
        default="cnn",
    )
